...
class SKLoader(GenericLoader):
	...
	functions = ['document', 'layer', 'masterlayer', ('bezier', 'b'), 
...
#		layout((%g,%g),%d)
		...
		if format is None:
				layout = pagelayout.PageLayout(width = sizes[0], height = sizes[1], orientation = orientation)
		elif sizes is None:
			if format in papersizes:
				...
				self.add_message(_("Unknown paper format '%s', using A4 instead") % format)
				format = "A4"
				layout = pagelayout.PageLayout(format, orientation=orientation)
		else:
			if format in papersizes:
...
#		page()
	...
	def guidelayer(self, *args, **kw):
		while not issubclass(self.composite_class, Document):
			self.end_composite()
