...
# Select all objects in the current layer with the same line color as
...
def select_same_line_color(context):
	...
	if properties.HasLine():
		...
		for obj in layer.GetObjects():
			if obj.has_line:
				prop = obj.Properties()
				if prop.HasLine() and color == prop.line_pattern.Color():
					select.append(obj)
...
Sketch.Scripting.AddFunction('select_same_line_color',
